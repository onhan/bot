...
def start(bot, update):
    ...
    time.sleep(1)
    custom_keyboard = [['Учить слова'], ['Проверить себя']]
    reply_markup = telegram.ReplyKeyboardMarkup(custom_keyboard, one_time_keyboard=False)
    bot.send_message(chat_id=update.message.chat_id, text="Что ты хочешь делать?", reply_markup=reply_markup)
    return ACTION

def action(bot, update):
    if(update.message.text == 'Учить слова'):
        learn(bot, update)
    elif(update.message.text == 'Проверить себя'):
...
def answer_check(bot, update):
    ...
    if(update.message.text == words[correct_word]):
        ...
        bot.send_chat_action(chat_id=update.message.chat_id , action = telegram.ChatAction.TYPING)
        bot.send_message(chat_id=update.message.chat_id, text="*Некорректно,*" + " *правильный ответ:* " + words[correct_word], parse_mode=telegram.ParseMode.MARKDOWN)
        bot.send_message(chat_id=update.message.chat_id, text="Что ты хочешь делать?", reply_markup=reply_markup)
        return ACTION
...
def error(bot, update, error):
    logger.warning('Обновить "%s" вызванная ошибка "%s"', update, error)

def main():
    updater = Updater(token='766419607:AAE2izJ49fb1y9uOgammRldzGMSY0x2uUtI')
    ...
    conv_handler = ConversationHandler(
        ...
        states = {
            ACTION: [RegexHandler('^(Учить слова|Проверить себя)$', action)],
